...
# Global lock for federation tasks
...
@bot.on_message(OWNER_FILTER & filters.command("admins", prefixes="@"))
@bot.on_message(OWNER_FILTER & filters.command("b", prefixes=","))
...
async def perform_fed_task(
    ...
    """
    async with FBAN_TASK_LOCK:  # 🔒 Only one fed-task at a time
        total = 0
        failed = []
